...
Connect Check
	...
	${ipv6_sta}=   Run keyword and Return Status   Should Contain    ${wlan_ipv6}  ${ipv6}
...
Login to Radius PC
	...
	Open Connection   ${AAA_IP}
        SSHLibrary.Login   ${AAA_PC_USER}   ${AAA_PC_PWD}
        Write   echo Remote RADIUS Login
	${output}=   Read Until   Login
...
Get MACADDR of clients and add to Radius
	...
	:For   ${m}  in   @{mac}
	\	Log   ${m}
	\	Execute Command    echo ${m} Cleartext-Password := \\"${m}\\" >> ${AAA_PATH}/users
	Execute Command    killall ${AAA_NAME}
	Start Command    ${AAA_NAME} -xX > /home/mac1.txt
...
Get MACADDR of clients and add edpsk entries to Radius
	...
	:For   ${m}  in   @{mac}
	\	Log   ${m}
	\	Write    sed -e "/${m} Cleartext-Password := /Id" ${AAA_PATH}/users
	\       ${m} =    Convert MAC To Lower Case   ${m}
	\	${m1} =    Remove String   ${m}   :   
	\	Write    sed -i "/${m1} Cleartext-Password := /d" ${AAA_PATH}/users

	:For   ${m}  in   @{mac}
	\	Log   ${m}
	\	${m} =    Convert MAC To Lower Case   ${m}
	\	${m1} =     Remove String   ${m}   :   
	\	Write    sed -i "/${m1} User-Password:=/,+2d" ${AAA_PATH}/users
	...
	\	${m} =    Convert MAC To Lower Case   ${m}
	\	${m1} =    Remove String   ${m}   :  
	\	Execute Command    echo ${m1} User-Password:=${m1}$'\n\t'Ruckus-Dpsk=0x00${edpsk} >> ${AAA_PATH}/users
	Execute Command    killall ${AAA_NAME}
	Start Command    ${AAA_NAME} -xX > /home/mac1.txt
...
Get MACADDR of clients and remove edpsk entries to Radius
	...
	Login to Radius PC
	:For   ${m}  in   @{mac}
	\	Write   sed -i 'N;$!P;$!D;$d' ${AAA_PATH}/users
	...
	\	${m} =    Convert MAC To Lower Case   ${m}
	\	${m1} =    Remove String   ${m}   :   
	\	Execute Command    echo ${m1} Cleartext-Password := \\"${m1}\\" >> ${AAA_PATH}/users
	Execute Command    killall ${AAA_NAME}
	Start Command    ${AAA_NAME} -xX > /home/mac1.txt
...
Get MACADDR of clients and check in Radius
	...
	:For   ${m}  in   @{mac}
	\	Log   ${m}
	\	${out} =  Execute command   sudo sed -i"users" "/${m}/d" ${AAA_PATH}/users
