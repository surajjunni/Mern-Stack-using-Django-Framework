...
SZ SUITE SETUP
	...
	\	run keyword if   '${res}'=='False'   Move AP   ${zone_id}  ${AP_MACC[${index}]}	
    	Get Zone Details
    	Zone Configuration
    	#Create Open WLAN
    	Create WLAN Group
...
Create FTP Profile
    [Arguments]    ${FTP_HOST_NAME}    ${FTP_USERNAME}    ${FTP_PASSWORD}    ${FTP_SERVER_PATH}    ${FTP_PORT_NUMBER}    ${FTP_PROTOCOL}    ${domainId}=
    ${FTP_NAME}=    String.Generate Random String    10   [LETTERS]
    ${ftp_data}=    Create Dictionary    domainId=${domainId}    ftpHost=${FTP_HOST_NAME}    ftpName=${FTP_NAME}    ftpUserName=${FTP_USERNAME}    ftpPassword=${FTP_PASSWORD}    ftpPort=${FTP_PORT_NUMBER}    ftpRemoteDirectory=${FTP_SERVER_PATH}    ftpProtocol=${FTP_PROTOCOL}
    ${ftp_profile_id}=    ftps create    ${ftp_data}
    Log    ${ftp_profile_id}
    Set Suite Variable    ${ftp_profile_id}
    Set Suite Variable    ${FTP_NAME}

Disable Auto Export Backup
    ${auto_export_data}=    Create Dictionary    enableAutoExportBackup=${False}
    modify_auto_export_backup    ${auto_export_data}

Delete FTP Profile
    [Arguments]    ${ftp_profile_id}
    Disable Auto Export Backup
    ${urlParams}=    Create Dictionary    ftpId=${ftp_profile_id}
    ftps delete_ftp_service_by_id    urlParams=${urlParams}

Auto Export Backup To FTP Server
    [Arguments]    ${FTP_NAME}
    ${auto_export_data}=    Create Dictionary    enableAutoExportBackup=${True}    ftpServer=${FTP_NAME}
    modify_auto_export_backup    ${auto_export_data}
